...
(function($) {
	"use strict";
		var isOn=0,sets, fx, toAnimate = "#effect", settings = {
			...
			easing: "easeOutBounce", //这里的效果 可以参见http://code.ciaoca.com/jquery/easing/   
			speed: 1000, //动画帧速 即在指定时间内完成动画
			sequenceDelay: 300, //场景延迟  即执行完上一个动画后，等待指定时间继续执行下一动画
			startDelay: 0, //开始延迟，即执行当前动画时，等待指定时间后再开始
			offsetX: 0,//动画元素 x坐标偏移量
			offsetY: 0,//动画元素 Y坐标偏移量
			onComplete: effect, //回调函数 即 执行完动画后，需要执行的回调方法
